...
class LongPicEditVerticalLogic extends GetxController {
  late final List<dynamic> _rawPhotos;
  final String mode;
  LongPicEditVerticalLogic() : mode = _getModeFromArgs();
  static String _getModeFromArgs() {
    final args = Get.arguments as Map<String, dynamic>?;
    return args?['mode'] as String? ?? 'vertical';
  }
  ...
  Future<void> saveAsTemplate(String templateName) async {
    try {
      final template = EditTemplate(
        name: templateName,
        borderColor: borderConfig.value.color.value,
        padding: borderConfig.value.padding,
      );
      final success = await UserPreferences.to.saveVerticalTemplate(template);
      if (success) {
        successToast('Template saved successfully');
      } else {
        errorToast('Failed to save template');
      }
    } catch (e) {
      errorToast('Failed to save template: ${e.toString()}');
    }
  }
  void loadTemplate() {
    try {
      ...
      if (template == null) {
        errorToast('No template found');
        return;
      }
      borderConfig.value = BorderConfig(
        color: Color(template.borderColor),
        padding: template.padding,
      );
      _generatePreviewDebounced();
      successToast('Template applied: ${template.name}');
    } catch (e) {
  ...
  Future<void> deleteTemplate() async {
    try {
      await UserPreferences.to.saveVerticalTemplate(
        EditTemplate(
          name: '',
  ...
  Future<void> _deduplicatePhotos() async {
    try {
      ...
      for (final data in imageDataList) {
        ...
        final hash = '${data.length}_${data.first}_${data.last}_'
            '${data.length > 100 ? data[mid] : 0}_'
            '${data.length > 1000 ? data[data.length ~/ 4] : 0}';
      ...
      if (unique.length < before) {
        ...
        successToast(
            'Removed $removed duplicate photo${removed > 1 ? 's' : ''}');
      } else {
        successToast('No duplicates found');
      }
    } catch (e) {
      errorToast('Deduplication failed: ${e.toString()}');
    }
  }
  ...
  List<Uint8List> _getOrderedImageData() {
    debugPrint('=== _getOrderedImageData 开始 ===');
  ...
  static Uint8List? _stitchInIsolate(_StitchParams params) {
    try {
      print('=== 开始拼接，输入图片数量: ${params.imageDataList.length} ===');
      ...
      if (params.isVertical) {
        ...
        for (int i = 0; i < images.length; i++) {
          ...
          if (image.width != maxWidth) {
            ...
            print('需要缩放图片 $i 到: ${maxWidth}x${newHeight}');
            final resized =
                img.copyResize(image, width: maxWidth, height: newHeight);
            resizedImages.add(resized);
            print('图片 $i 缩放完成');
          } else {
            print('图片 $i 宽度已是最大，无需缩放');
            resizedImages.add(image);
          }
        }
        print('=== 所有图片已处理，resizedImages 数量: ${resizedImages.length} ===');
        final canvasW = maxWidth + padding * 2;
        final totalHeight =
            resizedImages.map((e) => e.height).reduce((a, b) => a + b);
        final canvasH =
            totalHeight + padding * 2 + padding * (resizedImages.length - 1);
        ...
        for (int i = 0; i < resizedImages.length; i++) {
          ...
          img.compositeImage(canvas, image, dstX: padding, dstY: cy);
          cy += image.height;
        ...
        for (final image in images) {
          if (image.height != maxHeight) {
            ...
            final resized =
                img.copyResize(image, width: newWidth, height: maxHeight);
            resizedImages.add(resized);
          } else {
            resizedImages.add(image);
          }
        ...
        final totalWidth =
            resizedImages.map((e) => e.width).reduce((a, b) => a + b);
        final canvasW =
            totalWidth + padding * 2 + padding * (resizedImages.length - 1);
        final canvas = img.Image(width: canvasW, height: canvasH);
        img.fill(canvas, color: bg);
        int cx = padding;
        for (int i = 0; i < resizedImages.length; i++) {
  ...
  void onRotateTap() async {
    ...
    try {
      ...
      final file = File(
          '${dir.path}/long_pic_rotated_${idx}_${DateTime.now().millisecondsSinceEpoch}.jpg');
  ...
  Future<void> onSaveTap() async {
    ...
    try {
      ...
      final result = await ImageGallerySaverPlus.saveImage(
        capturedImage,
        quality: 100,
        name: 'LongPic_Vertical_${DateTime.now().millisecondsSinceEpoch}',
      );
      if (result['isSuccess'] == true || result['filePath'] != null) {
        await _saveHistory(capturedImage, mode);
  ...
  Future<Uint8List?> _generateCompleteImage() async {
    ...
    final result = await compute(_stitchInIsolate, params);
    debugPrint('=== 完整长图生成${result != null ? '成功' : '失败'} ===');
    return result;
  }
  Future<void> _saveHistory(Uint8List imageData, String type) async {
    try {
      ...
      String displayType;
      if (type == 'smart') {
        displayType = 'Smart Stitch';
      } else if (type == 'vertical') {
        displayType = 'Vertical Stitch';
      } else {
        displayType = 'Horizontal Stitch';
      }
      final record = HistoryRecord(
        type: type,
        title:
            '$displayType - ${imageDataList.length} photo${imageDataList.length > 1 ? 's' : ''}',
        thumbnail: thumbPath,
